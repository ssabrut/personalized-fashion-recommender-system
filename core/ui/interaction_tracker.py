...
class InteractionType(Enum):
    ...
    def from_str(cls, value: str) -> "InteractionType":
        return {"purchase": cls.PURCHASE, "click": cls.CLICK, "ignore": cls.IGNORE}[
            value.lower()
        ]
...
class InteractionTracker:
    ...
    def track(self, customer_id: str, article_id: str, interaction_type: str):
        ...
        if int_type == InteractionType.PURCHASE:
            ...
            logger.info(
                f"Tracked purchase of item {article_id} by customer {customer_id}"
            )
    ...
    def _add_interaction(
        self, customer_id, article_id, interaction_type, prev_article_id, timestamp=None
    ):
