        ...
        for row in range(num_rows):
            ...
            for idx, item_data in enumerate(row_items):
                if tracker.should_show_item(customer_id, item_data[1][0]):
                    with cols[idx]:
                        if display_llm_item(
                            item_data, cols[idx], articles_fv, customer_id, tracker
                        ):
                            need_rerun = True
                        else:
...
def llm_recommendations(articles_fv, api_key, customer_id):
    ...
    selected_input = st.selectbox(
        "Choose your fashion need or enter a custom one:", input_options
    )
            ...
            try:
                ...
                for emoji, category, description in item_recommendations:
                    similar_items = get_similar_items(
                        description, embedding_model, articles_fv
                    )
                    shown_items = []
                    extra_items = []
                    ...
                    for item in similar_items:
                        if len(shown_items) < 5 and tracker.should_show_item(
                            customer_id, item[0]
                        ):
                    ...
                    if shown_items:
                        st.session_state.llm_recommendations.append(
                            (emoji, category, shown_items)
                        )
                        st.session_state.llm_extra_items[category] = extra_items

                        # Track shown items
                        tracker.track_shown_items(
                            customer_id, [(item[1][0], 0.0) for item in shown_items]
                        )
        ...
        for idx, item_data in enumerate(items):
            ...
            if tracker.should_show_item(customer_id, item_id):
                with cols[idx % n_cols]:
                    ...
                    was_purchased = display_llm_item(
                        item_data, cols[idx % n_cols], articles_fv, customer_id, tracker
                    )
