...
def generate_interaction_data(trans_df: pl.DataFrame) -> pl.DataFrame:
    ...
    def generate_timestamp(base_timestamp, count, min_hours, max_hours):
        hours = np.random.randint(min_hours, max_hours, size=count)
        return base_timestamp - (hours * 3_600_000)
    ...
    for chunk_start in tqdm(
        ...
        for customer_id in chunk_customers:
            customer_purcahses = chunk_transactions.filter(
                pl.col("customer_id") == customer_id
            )

            if len(customer_purcahses) == 0:
                continue

            customer_articles = {"purchased": set(), "clicked": set(), "ignored": set()}
            last_purchase_timestamp = customer_purcahses["t_dat"].max()

            # generate ignores first
            ...
            purchase_rows = customer_purcahses.iter_rows(named=True)
            for row in purchase_rows:
                ...
                if np.random.random() < CLICK_BEFORE_PURCHASE_PROB:
                    ...
                    for _ in range(num_pre_clicks):
                        click_timestamp = generate_timestamp(
